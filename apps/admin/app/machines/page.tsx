...
export default function MachinesPage() {
  ...
  const handleSyncFromOracle = async () => {
    setSyncing(true);
    try {
      const result = await syncMachinesFromOracle();

      if (result.success) {
        toast.success(
          `Oracle 동기화 완료\n` +
          `총 ${result.total_oracle_machines}개 중 ` +
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">생산 설비 및 장비를 관리합니다</p>
        </div>
            ...
            <Table>
              <TableHeader className="bg-gray-800">
                <TableRow className="hover:bg-gray-800">
                  ...
                  <TableHead className="text-gray-400">설비 이름</TableHead>
                  <TableHead className="text-gray-400">설비 코드</TableHead>
                  <TableHead className="text-gray-400">위치</TableHead>
                ...
                {filteredMachines.length === 0 ? (
                  ...
                  filteredMachines.map((machine) => (
                    ...
                    >
                      ...
                      <TableCell className="text-white font-semibold">{machine.machine_name}</TableCell>
                      <TableCell className="text-gray-300">
                        <span className="px-2 py-1 bg-blue-500/10 text-blue-400 rounded text-xs font-mono">
                          {machine.machine_code}
                        </span>
                      </TableCell>
                      <TableCell className="text-gray-400">{machine.location || '-'}</TableCell>
                      <TableCell>
                        {machine.enabled ? (
                      ...
                      <TableCell className="text-gray-400">
                        {new Date(machine.created_at).toLocaleDateString('ko-KR')}
                      </TableCell>
